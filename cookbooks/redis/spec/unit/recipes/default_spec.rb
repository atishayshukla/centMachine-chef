  ...
  context 'When all attributes are default, on an unspecified platform' do
    let(:chef_run) do
      runner = ChefSpec::ServerRunner.new(step_into: ['redis']) do |node, server|
        node.default['redis']['version'] = version
       # This is needed to step into custom resource refer chefspec on github
      # Lets make use of node attribute
     end
    ...
    # For our custom resource lets write a test

    it 'installs redis' do
      expect(chef_run).to install_redis("#{version}")
    end

    # undefined method `install_redis' This fails so we write custom mactcher # https://github.com/sethvargo/chefspec
